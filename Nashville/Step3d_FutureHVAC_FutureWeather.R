...
path_name1 <- paste(directory_name, 'RCM_Building_Coefficients/', city_name, '_coeff.csv', sep="")

cooler_year_data <- list()
cooler_column_sums_list <- list()

hotter_year_data <- list()
hotter_column_sums_list <- list()
...
for (year in c(2050)) {
    ...
    for (scenario in c('rcp45cooler', 'rcp85cooler')) {
      print(paste("scenario is ", scenario, sep=""))
      test_name <- ifelse(scenario == 'rcp45cooler', 'test_cooler_', 'test_hotter_')
      
      #weather_name <- paste(directory_name, '/Weather_CSV/', scenario, '/',city_name,'/',test_name, year,'.csv', sep="")
      
      
      weather_name <- paste(directory_name, '/Weather_CSV/Energy_Burdens/', city_name, '_', scenario, '_2020-2060_2050.csv', sep="")
      ...
      colnames(agg_data) <- c('y_pred', 'elec_pred', 'cooling_pred','heating_pred', 'cooling_elec','heating_elec', 'baseline')
        for (building in building_coeff$building_id) {
          ...
          new_test$solar  <- weather_data$p_Solar
    ...
    scenario_predictions[[as.character(building)]] <- fuel_consumption_df
    ...
    i = i+1
      ...
      year_predictions[[scenario]] <- scenario_predictions
      if (scenario == 'rcp45cooler'){
        cooler_year_data[[as.character(year)]] <- agg_data
        column_sums <- colSums(abs(agg_data))
        cooler_column_sums_list[[as.character(year)]] <- column_sums
      }else{
        hotter_year_data[[as.character(year)]] <- agg_data
        column_sums <- colSums(abs(agg_data))
        hotter_column_sums_list[[as.character(year)]] <- column_sums
      }
    }
    all_predictions[[as.character(year)]] <- year_predictions
  
}

#saveRDS(all_predictions, "predictions_FHFW.rds")
# Define the path variable
#output_path <- "C:/Users/yangl11/R_Project/ACS_DATA/Detroit/"

# Save the RDS file using the variable
saveRDS(all_predictions, file.path(output_path, "predictions_FHFW.rds"))
print("file saved")
