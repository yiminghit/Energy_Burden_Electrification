...
file_path_top10 <- file.path(output_path, "Figures", paste(city, "_top_10_4months.png", sep=""))
...
months_to_plot <- c(1, 4, 7, 10)  # January, April, July, October
...
for (i in seq_along(months_to_plot)) {
  ...
  q0s <- tapply(top_10_data_combined$Value, top_10_data_combined$scenario, quantile, probs = 0.05, na.rm = TRUE)
  q4s <- tapply(top_10_data_combined$Value, top_10_data_combined$scenario, quantile, probs = 0.95, na.rm = TRUE)
...
top10_summary_csv_path <- file.path(output_path, "top10_montly_summary_stats.csv")
...
combined_plot <- grid.arrange(grobs = top10_plot_list, ncol = 4)
...
ggsave(file_path_top10, plot = final_plot, width = 20, height = 3, dpi = 400)
