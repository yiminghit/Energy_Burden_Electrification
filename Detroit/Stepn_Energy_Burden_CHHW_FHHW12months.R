...
save_path <- file.path(routine_path, city_to_run, "CHHW_FHHW12months.rds")


# Read the RDS files
bill_list_1 <- readRDS(file.path(output_path, "CHHW12bill.rds"))

bill_list_2  <- readRDS(file.path(output_path, "FHHW12bill.rds"))
...
hours_in_month <- c(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 30)*24
...
for (house in results_setback$`_id_base`) {
  ...
  #bill_sums_vector<- calculate_energy_bill_sums_vector(results_setback, predictions_list, endpoints,i)
  
  bill_sums_vector1 <- bill_list_1[[i]]  
  bill_sums_vector2 <- bill_list_2[[i]] 
  bill_sums_vector =  bill_sums_vector2-  bill_sums_vector1
