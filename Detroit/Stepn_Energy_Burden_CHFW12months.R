...
save_path <- file.path(routine_path, city_to_run, "CHFW12months.rds")
save_path2 <- file.path(routine_path, city_to_run, "CHFW12bill.rds")
###
## Base data wrangling
###
...
file_name2 <- 'predictions_CHFW.rds'
...
for (year in c(2050)) {
  ...
  for (scenario in c('rcp45cooler', 'rcp85cooler')) {
    ...
    combined_data_list <- vector("list", length = 12) 
    for (ii in 1:12) {
      combined_data_list[[ii]] <- numeric(0)
    }
    ...
    for (house in results_setback$`_id_base`) {
      ...
      #electricity_bill<- sum(h_8760_setback_df$total_site_electricity_kwh* 0.1072)
      ...
      bill_sums_vector <- calculate_energy_bill_sums_vector( results_setback, scenario_predictions, endpoints,i)
      bill_sums_vectors_list[[i]] <- bill_sums_vector
      ####
...
summary_scenario[[scenario]] <-  combined_data_list
print(paste("ProgressSSSSSSSSSSS "))
  }
