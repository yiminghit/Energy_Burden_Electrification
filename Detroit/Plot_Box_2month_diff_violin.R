...
file_path1 <- file.path(output_path,  "Figures", paste(city,"_box_plot_2months_all_diff.png", sep=""))
...
for (i in seq_along(months_to_plot)) {
  ...
  boxplot_list[[month_name]] <- ggplot(boxplot_data, aes(x = scenario, y = Value, fill = scenario, group = scenario)) +
    geom_violin(trim = FALSE, scale = "width") + # Violin plot with full range and consistent width
    ggtitle(paste(month_name)) +
    scale_fill_manual(values = color_mapping) + # Apply dynamic color mapping
    coord_cartesian(ylim = c(-0.05, 0.05)) +
    stat_summary(fun = "median", geom = "crossbar", width = 0.4, color = "black", fatten = 2) +  
  ...
  ggsave(
    filename = file.path(output_path, paste0("boxplot_", month_name, ".png")),
