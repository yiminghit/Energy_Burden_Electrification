calculate_energy_bill <- function(results_setback, scenario_predictions,i) {
  ...
  gas_bill <- sum(( fuel_consumption_df$Natural_Gas+  fuel_consumption_df$Natural_Gas_Base) / 10.38 * 16.52) #One thousand cubic feet is 10.38 thermal we use 2022 gas price
  propane_bill <- sum(( fuel_consumption_df$Propane+  fuel_consumption_df$Propane_Base) / 91.452 * 2.88)
  ...
  total_electricity_bill <- sum((total_electric)* 0.1431)
