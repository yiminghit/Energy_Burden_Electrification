...
def get_table(df, targets):
  ...
  #  shape = tuple([df[c].unique().shape[0] for c in factors])
   # aaa=cross_tab.values
    ...
    ini_data = robjects.r['readRDS']('Philadelphia_income_ini.rds')
...
f, u_name, u, X = get_table(df, {
    'House Size': {'0-1499':0.429897410845139, '1500-2499':0.357162242848613, '2500-3499':0.136242740053194, '3500+':0.0766976062530533},

    #'House Bedroom': {'1': 0.09726210989, '2': 0.21740379092,'3':0.42188397472,'4 or more': 0.26345012445},
    'AC type': {'Central':0.656905215733842, 'Room':0.314156991855831, 'None':0.0289377924103275},
    'Income':{'<15k':0.113019870235739, '15k-34999':0.164462289508052, '35k-74999':0.273481664507775, '75k-99999':0.122300220415528, '>100k':0.326735955332906
}
})
...
df.to_csv('./Philadelphia_random.csv')