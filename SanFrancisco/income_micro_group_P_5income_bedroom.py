...
def get_table(df, targets):
  ...
  #  shape = tuple([df[c].unique().shape[0] for c in factors])
   # aaa=cross_tab.values
    #table=np.ones((8,4,3,9,22))

    ini_data = robjects.r['readRDS']('SanFrancisco_income_ini.rds')
    tensor_data = np.array(ini_data)
    table = tensor_data
    #table = np.random.rand(9, 9, 3, 9, 22)
...
f, u_name, u, X = get_table(df, {
    'House Size': {'0-1499':0.591353122833554, '1500-2499':0.294258523980589, '2500-3499':0.0787168336799647, '3500+':0.0356715195058927},

    #'House Bedroom': {'1': 0.09726210989, '2': 0.21740379092,'3':0.42188397472,'4 or more': 0.26345012445},
    'AC type': {'Central':0.325870355175243, 'Room':0.0713867073027781, 'None':0.602742937521979},
    'Income':{'<15k':0.0811285205402828, '15k-34999':0.119867156033469, '35k-74999':0.215669555086721, '75k-99999':0.114236785522787, '>100k':0.469097982816741
}
})
...
df.to_csv('./SanFrancisco_random.csv')