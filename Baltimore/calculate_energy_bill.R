calculate_energy_bill <- function(results_setback, scenario_predictions,i) {
  ...
  gas_bill <- sum(( fuel_consumption_df$Natural_Gas+  fuel_consumption_df$Natural_Gas_Base) / 10.38 * 16.97) #One thousand cubic feet is 10.38 thermal we use 2022 gas price
  propane_bill <- sum(( fuel_consumption_df$Propane+  fuel_consumption_df$Propane_Base) / 91.452 * 3.39)
  oil_bill <- sum((fuel_consumption_df$Fuel_Oil+fuel_consumption_df$Fuel_Oil_Base) / 137.381 * 4.46)
  
  total_electric <- fuel_consumption_df$Electricity+fuel_consumption_df$Electricity_Base
  
  total_electricity_bill <- sum((total_electric)* 0.15644)+9.3*12 #0.04588+0.11056
