...
path_name2 <- paste(directory_name, 'RCM_Building_Coefficients/', city_name, '_coeff.csv', sep="")
...
for (year in c(2050)) {
  ...
  for (scenario in c('rcp45cooler', 'rcp85cooler')) {
    ...
    for (building in building_id$building_id) {
      path_name3 <- paste(directory_name, 'TrainTest_Data/', city_name, '/test_', building,'.csv', sep="")
      test_data <- read.csv(path_name3,header=TRUE)
      ...
      heating_COP[results_base_df$building_characteristics_report.hvac_system_heat_pump[building_id$building_id == building] == "ASHP, SEER 13, 7.7 HSPF"] <- 4.4 - (0.063*new_test$delta_kelvin) 
...
saveRDS(all_predictions, file.path(output_path, "predictions_CHFW.rds"))
print("file saved")
