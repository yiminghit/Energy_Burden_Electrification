...
def get_table(df, targets):
  ...
  #  shape = tuple([df[c].unique().shape[0] for c in factors])
   ...
   # ini_data = robjects.r['readRDS']('Buffalo_income_ini.rds')
   # tensor_data = np.array(ini_data)
   # table = tensor_data
   # table = np.random.rand(4, 3, 5)
    np.random.seed(30)  # Set seed to 42
    table = np.random.rand(4, 3, 5)
    #table = np.random.normal(loc=0, scale=1, size=(4, 3, 5))

    # Normalize along the last axis (axis=-1) so that numbers in each row sum to 1
...
f, u_name, u, X = get_table(df, {
    'House Size': {'0-1499':0.464755077658304, '1500-2499':0.353345280764636, '2500-3499':0.1084229390681, '3500+':0.0734767025089606},
    #'House Bedroom': {'1': 0.09726210989, '2': 0.21740379092,'3':0.42188397472,'4 or more': 0.26345012445},
    'AC type': {'Central':0.561349693251534, 'Room':0.438650306748466, 'None':0.00001},
    'Income':{'<15k':0.133450265521799, '15k-34999':0.205509023946504, '35k-74999':0.306373577260625, '75k-99999':0.126651473914776, '>100k':0.228015659356297
}
})
