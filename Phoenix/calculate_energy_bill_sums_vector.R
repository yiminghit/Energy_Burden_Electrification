calculate_energy_bill_sums_vector <- function(results_setback, scenario_predictions, endpoints,i) {
  ...
  rates <- list(
    May = list(on_peak = 0.2270, off_peak = 0.0903),
    June = list(on_peak = 0.2270, off_peak = 0.0903),
    July = list(on_peak = 0.2585, off_peak = 0.0906),
    August = list(on_peak = 0.2585, off_peak = 0.0906),
    September = list(on_peak = 0.2270, off_peak = 0.0903),
    October = list(on_peak = 0.2270, off_peak = 0.0903),
    November = list(on_peak = 0.1145, off_peak = 0.0855),
    December = list(on_peak = 0.1145, off_peak = 0.0855),
    January = list(on_peak = 0.1145, off_peak = 0.0855),
    February = list(on_peak = 0.1145, off_peak = 0.0855),
    March = list(on_peak = 0.1145, off_peak = 0.0855),
    April = list(on_peak = 0.1145, off_peak = 0.0855)
  )
  ...
  # Split gas segment8
  gas_segments <- split((fuel_consumption_df$Natural_Gas+  fuel_consumption_df$Natural_Gas_Base) / 10.38 * 17.81, findInterval(seq_along((fuel_consumption_df$Natural_Gas+  fuel_consumption_df$Natural_Gas_Base) / 10.38 * 17.81), endpoints))
  gas_segment_sums <- lapply(gas_segments, sum)
  gas_sums_vector <- unlist(gas_segment_sums)
  
  propane_segments <- split((fuel_consumption_df$Propane+  fuel_consumption_df$Propane_Base) / 91.452 * 2.97, findInterval(seq_along((fuel_consumption_df$Propane+  fuel_consumption_df$Propane_Base) / 91.452 * 2.97), endpoints))
  propane_segment_sums <- lapply(propane_segments, sum)
  propane_sums_vector <- unlist(propane_segment_sums)
  
  oil_segments <- split((fuel_consumption_df$Fuel_Oil+fuel_consumption_df$Fuel_Oil_Base) / 137.381 * 4.29, findInterval(seq_along((fuel_consumption_df$Fuel_Oil+fuel_consumption_df$Fuel_Oil_Base) / 137.381 * 4.29), endpoints))
  ...
  for (day_index in 1:num_days) {
    ...
    if (month %in% 5:10) { # April - September (summer)
      peak <- sum(daily_electric[14:20])* rate$on_peak  # kWh for lower rate
      offpeak <- (daily_sum -sum(daily_electric[14:20]))*rate$off_peak
    } else { # October - March (winter)
      peak <- sum(daily_electric[c(5:8, 17:20)])* rate$on_peak
      offpeak <- (daily_sum -sum(daily_electric[c(5:8, 17:20)]))*rate$off_peak
    }
  ...
  energy_bill <- round(electricity_bill + gas_sums_vector+ propane_sums_vector+ oil_sums_vector, 2)+20
