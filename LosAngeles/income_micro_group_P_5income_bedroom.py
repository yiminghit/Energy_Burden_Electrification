...
def get_table(df, targets):
  ...
  #  shape = tuple([df[c].unique().shape[0] for c in factors])
   # aaa=cross_tab.values
    ...
    ini_data = robjects.r['readRDS']('LosAngeles_income_ini.rds')
...
f, u_name, u, X = get_table(df, {
    'House Size': {'0-1499':0.610364479787939, '1500-2499':0.277190192180252, '2500-3499':0.0694102054340623, '3500+':0.0430351225977469},

    #'House Bedroom': {'1': 0.09726210989, '2': 0.21740379092,'3':0.42188397472,'4 or more': 0.26345012445},
    'AC type': {'Central':0.544248794248794, 'Room':0.232459732459732, 'None':0.223291473291473},
    'Income':{'<15k':0.105773980399864, '15k-34999':0.173518011080109, '35k-74999':0.277009675999161, '75k-99999':0.120363902009502, '>100k':0.323334430511364
}
})
...
df.to_csv('./LosAngeles_random.csv')