...
def get_table(df, targets):
  ...
  #  shape = tuple([df[c].unique().shape[0] for c in factors])
   # aaa=cross_tab.values
    ...
    ini_data = robjects.r['readRDS']('NewYorkCity_income_ini.rds')
...
f, u_name, u, X = get_table(df, {
    'House Size': {'0-1499':0.4395226072986, '1500-2499':0.386274959834749, '2500-3499':0.109364241450539, '3500+':0.064838191416112},

    #'House Bedroom': {'1': 0.09726210989, '2': 0.21740379092,'3':0.42188397472,'4 or more': 0.26345012445},
    'AC type': {'Central':0.925024925224327, 'Room':0.0689930209371884, 'None':0.00598205383848455},
    'Income':{'<15k':0.155096429618753, '15k-34999':0.215058951742555, '35k-74999':0.293525395316721, '75k-99999':0.110915379447898, '>100k':0.225403843874073
}
})
...
df.to_csv('./NewYorkCity_random.csv')