...
save_path <- file.path(routine_path, city_to_run, "CHHW_FHHW_one_year.rds")
#save_path2 <- file.path(routine_path, city_to_run, "CHHW_one_year_bill.rds")

bill_list_1 <- readRDS(file.path(output_path, "CHHW_one_year_bill.rds"))

bill_list_2  <- readRDS(file.path(output_path, "FHHW_one_year_bill.rds"))
###
...
energy_bill_df <- data.frame(house_id = character(), energy_bill = numeric(), fuel_type= numeric(), stringsAsFactors = FALSE)
...
for (house in results_setback$`_id_base`) {
  ...
  #energy_bill <- calculate_energy_bill(results_setback, predictions_list,i)
 # bill_sums_vectors_list[[i]] <-  energy_bill
  bill_sums_vector1 <- bill_list_1[[i]]  
  bill_sums_vector2 <- bill_list_2[[i]] 
  energy_bill =  bill_sums_vector2-  bill_sums_vector1
  
  print( energy_bill)
  # Specify the path to your JSON file
 # json_file_path <- "results.json"
  energy_bill_df <- rbind(energy_bill_df, data.frame(house_id = results_setback$build_existing_model.building_id[i], energy_bill = round(energy_bill, 2), fuel_type=results_setback$building_characteristics_report.heating_fuel[i]))
  ...
  if (range[1] > 0) {
    ...
    data_case <- c(
      runif(floor(sample_size * target_row[5]), range[4], 0),
      runif(floor(sample_size * target_row[4]), range[3], range[4]),
      runif(floor(sample_size * target_row[3]), range[2], range[3]),
      runif(floor(sample_size * target_row[2]), range[1], range[2]),
      runif(floor(sample_size * target_row[1]), lower_bound, range[1])
    )
  }
