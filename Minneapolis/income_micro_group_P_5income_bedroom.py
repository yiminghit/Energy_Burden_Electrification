...
def get_table(df, targets):
  ...
  #  shape = tuple([df[c].unique().shape[0] for c in factors])
   # aaa=cross_tab.values
    ...
    ini_data = robjects.r['readRDS']('Minneapolis_income_ini.rds')
...
f, u_name, u, X = get_table(df, {
    'House Size': {'0-1499':0.389156728410725, '1500-2499':0.362936648904677, '2500-3499':0.149031548676309, '3500+':0.0988750740082889},

    #'House Bedroom': {'1': 0.09726210989, '2': 0.21740379092,'3':0.42188397472,'4 or more': 0.26345012445},
    'AC type': {'Central':0.754928964917367, 'Room':0.218976514931864, 'None':0.0260945201507683},
    'Income':{'<15k':0.0744262461979059, '15k-34999':0.145851392435133, '35k-74999':0.288410280169059, '75k-99999':0.140774310348262, '>100k':0.35053777084964
}
})
...
df.to_csv('./Minneapolis_random.csv')